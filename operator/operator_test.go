...
func TestReplaceWithMultiple(t *testing.T) {
	...
	Test(t, "ReplaceWithMultiple: replaces one statement with several", func(t *T) {
		...
		t.Equal(len(block.List), 3)
		if len(block.List) == 3 {
			t.Ok(block.List[0] == one)
			t.Ok(block.List[1] == two)
		}
		t.End()
	})
}
...
func TestIsSimple(t *testing.T) {
	Test(t, "IsSimple: literals, identifiers and selectors are simple", func(t *T) {
		...
		t.Ok(IsSimple(ast.NewIdent("x")))
		t.Ok(IsSimple(&ast.SelectorExpr{X: ast.NewIdent("a"), Sel: ast.NewIdent("b")}))
		t.End()
	})
