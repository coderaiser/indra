...
func TestMergeIfStatements(t *testing.T) {
	Test(t, "conditions: merge-if-statements: report", func(t *T) {
		t.Report("merge-if-statements", "merge if statements")
		t.End()
	})

	Test(t, "conditions: merge-if-statements: transform", func(t *T) {
		t.Transform("merge-if-statements")
		t.End()
	})

	Test(t, "conditions: merge-if-statements: no report: no-merge-outer-else", func(t *T) {
		t.NoReport("no-merge-outer-else")
		t.End()
	})

	Test(t, "conditions: merge-if-statements: no report: no-merge-inner-else", func(t *T) {
		t.NoReport("no-merge-inner-else")
		t.End()
	})

	Test(t, "conditions: merge-if-statements: no report: no-merge-multi-stmt", func(t *T) {
