...
func TestConvertNoErrorToNotOk(t *testing.T) {
	...
	Test(t, "convert-no-error-to-not-ok: fixes NoError to NotOk convert-no-error-to-not-ok", func(t *T) {
		t.Transform("convert-no-error-to-not-ok")
		t.End()
	})

	Test(t, "convert-no-error-to-not-ok: no report for NotOk not-ok", func(t *T) {
		t.NoReport("not-ok")
		t.End()
	})

	Test(t, "convert-no-error-to-not-ok: no report without go-tape import no-error-other-lib", func(t *T) {
		t.NoReport("no-error-other-lib")
		t.End()
	})

	Test(t, "convert-no-error-to-not-ok: no transform without go-tape import no-error-other-lib", func(t *T) {
