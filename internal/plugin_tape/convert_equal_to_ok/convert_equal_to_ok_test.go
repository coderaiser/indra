...
func TestConvertEqualToOk(t *testing.T) {
	Test(t, "convert-equal-to-ok: report Equal(x, true)", func(t *T) {
		t.Report("convert-equal-to-ok", "convert Equal(x, true) to Ok(x)")
		t.End()
	})

	Test(t, "convert-equal-to-ok: transform Equal(x, true) to Ok", func(t *T) {
		t.Transform("convert-equal-to-ok")
		t.End()
	})

	Test(t, "convert-equal-to-ok: no report for Ok", func(t *T) {
		t.NoReport("ok")
		t.End()
	})

	Test(t, "convert-equal-to-ok: no transform for Ok", func(t *T) {
		t.NoTransform("ok")
		t.End()
	})
}
